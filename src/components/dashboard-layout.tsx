...
export default function DashboardLayout({ children, currentPath }: Props) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-4rem)]">
      <aside className="hidden w-64 shrink-0 border-r border-warm-200 bg-cream-50 p-4 dark:border-cream-200 dark:bg-cream-50 md:block">
        <nav className="space-y-1">
          {navItems.map((item) => {
            const isActive = currentPath === item.path;
            return (
              <a key={item.path} href={item.href} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${isActive ? "bg-blush-100 text-rose-gold-700 dark:bg-rose-gold-900 dark:text-blush-200" : "text-warm-600 hover:bg-cream-100 hover:text-warm-900 dark:text-warm-600 dark:hover:bg-cream-100 dark:hover:text-warm-800"}`}>
        ...
        <div className="mt-8 border-t border-warm-200 pt-4 dark:border-cream-200">
          <form action="/api/auth/logout" method="POST">
            <button type="submit" className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-warm-600 hover:bg-cream-100 hover:text-warm-900 dark:text-warm-600 dark:hover:bg-cream-100 dark:hover:text-warm-800">
              <svg className="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="flex-1">
        ...
        {dbStatus === "checking" && (
          <div className="flex items-center gap-2 border-b border-warm-200 bg-cream-50 px-6 py-2 text-xs text-warm-500 dark:border-cream-200 dark:bg-cream-50">
        ...
        <div className="flex overflow-x-auto border-b border-warm-200 md:hidden dark:border-cream-200">
          {navItems.map((item) => {
            const isActive = currentPath === item.path;
            return (
