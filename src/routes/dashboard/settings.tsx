    ...
    <DashboardLayout currentPath="settings">
      <div className="p-6 sm:p-8">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage your account and billing.</p>
        </div>
        ...
        <div className="space-y-8">
          <section className="rounded-xl border border-gray-200 p-6 dark:border-gray-800">
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Your account information.</p>
            <div className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
                <input type="text" disabled className="mt-1 block w-full max-w-sm rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" placeholder="Your name" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                <input type="email" disabled className="mt-1 block w-full max-w-sm rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" placeholder="you@example.com" />
              </div>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-gray-200 p-6 dark:border-gray-800">
            ...
            {loading ? (
              <p className="mt-2 text-sm text-gray-500">Loading billing info…</p>
            ) : currentPlan ? (
              <div className="mt-2">
                <p className="text-sm text-gray-600 dark:text-gray-400">You&apos;re on the <strong>{currentPlan.name}</strong> plan{currentPlan.price > 0 && ` ($${currentPlan.price}/mo)`}.</p>
                {billing?.currentPeriodEnd && <p className="mt-1 text-xs text-gray-500">Current period ends: {new Date(billing.currentPeriodEnd).toLocaleDateString()}</p>}
            ...
            ) : <p className="mt-2 text-sm text-gray-500">No plan selected.</p>}
