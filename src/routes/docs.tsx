    ...
    <div className="py-16 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-lg text-warm-600 dark:text-warm-600">Everything you need to get the most out of LoomLink.</p>
        </div>
              ...
              <div className="mt-6 space-y-6">
                {section.items.map((item) => (
                  <div key={item.name}>
                    ...
                    <p className="mt-1 text-sm leading-6 text-warm-600 dark:text-warm-600">{item.desc}</p>
                    {item.details && (
        ...
        <div id="quickstart" className="mt-20 scroll-mt-20">
          ...
          <div className="mt-6 space-y-8">
            <div className="rounded-xl border border-warm-200 p-6 dark:border-cream-200">
              ...
              <p className="mt-3 text-sm text-warm-600 dark:text-warm-600">Go to the Integrations page in your dashboard. Pick a service (Stripe, Gmail, Slack, etc.) and click Connect. For API-key-based services, paste your key. For OAuth services, you'll be redirected to authorize access.</p>
            </div>
            <div className="rounded-xl border border-warm-200 p-6 dark:border-cream-200">
              <div className="flex items-center gap-3">
              ...
              <p className="mt-3 text-sm text-warm-600 dark:text-warm-600">Go to Workflows → New Workflow. Give it a name, choose a trigger (Manual, Schedule, or Webhook), then add action steps. Each step picks a connector and an operation. Save when you're done.</p>
            </div>
            <div className="rounded-xl border border-warm-200 p-6 dark:border-cream-200">
              <div className="flex items-center gap-3">
              ...
              <p className="mt-3 text-sm text-warm-600 dark:text-warm-600">From the Workflows list, click Run to test your workflow manually. Check the Activity log to see results. Once everything works, enable the workflow to run on its trigger schedule automatically.</p>
            </div>
          </div>
        </div>
        ...
        <div id="action-chaining" className="mt-20 scroll-mt-20">
          ...
          <p className="mt-2 text-sm text-warm-600 dark:text-warm-600">Use <code className="rounded bg-cream-100 px-1.5 py-0.5 text-xs font-mono dark:bg-cream-50">{`{{variable}}`}</code> syntax to reference data from triggers or previous steps.</p>
          <div className="mt-6 overflow-x-auto rounded-xl border border-warm-200 dark:border-cream-200">
            <table className="min-w-full divide-y divide-warm-200 text-sm dark:divide-warm-800">
              <thead className="bg-cream-50 dark:bg-cream-50">
              ...
              <tbody className="divide-y divide-warm-200 dark:divide-warm-800">
                <tr><td className="px-4 py-3 font-mono text-xs">{`{{trigger.event}}`}</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">The webhook event payload or manual run context</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">{`{{trigger.scheduled_at}}`}</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">ISO timestamp of when a scheduled workflow fired</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">{`{{step_name.output_key}}`}</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Output value from a previous action step</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">{`{{step_name.result}}`}</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Full result object from a previous step</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div id="scheduled-workflows" className="mt-20 scroll-mt-20">
          ...
          <p className="mt-2 text-sm text-warm-600 dark:text-warm-600">Standard cron syntax with 5 fields: minute hour day-of-month month day-of-week.</p>
          <div className="mt-6 overflow-x-auto rounded-xl border border-warm-200 dark:border-cream-200">
            <table className="min-w-full divide-y divide-warm-200 text-sm dark:divide-warm-800">
              <thead className="bg-cream-50 dark:bg-cream-50">
              ...
              <tbody className="divide-y divide-warm-200 dark:divide-warm-800">
                <tr><td className="px-4 py-3 font-mono text-xs">* * * * *</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Every minute</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">*/15 * * * *</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Every 15 minutes</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">0 * * * *</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Every hour at :00</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">0 9 * * *</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Every day at 9:00 AM</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">0 9 * * 1-5</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">Weekdays at 9:00 AM</td></tr>
                <tr><td className="px-4 py-3 font-mono text-xs">0 0 1 * *</td><td className="px-4 py-3 text-warm-600 dark:text-warm-600">First day of month at midnight</td></tr>
              </tbody>
            </table>
          </div>
        </div>

        <div className="mt-16 rounded-xl border border-warm-200 bg-cream-50 p-8 text-center dark:border-cream-200 dark:bg-cream-50">
          <h2 className="text-xl font-bold">Need help?</h2>
          <p className="mt-2 text-warm-600 dark:text-warm-600">Can&apos;t find what you&apos;re looking for? Reach out to our support team.</p>
